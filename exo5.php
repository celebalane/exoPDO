<?php
	...
	if(empty($erreur)){
		...
		$message['success'][]= 'le client est bien ajouté';
			...
			$statement=$pdo->query('SELECT id,lastName,firstName FROM clients');
			$identite=$statement->fetchAll();
			
			for($index=0; $index<=count($identite);$index ++){
				for($index2=0;$index<=count($identite[$index]);$index2 ++){
					if($donnee['lastName']==$identite['lastName'] && $donnee['firstName']==$identite['firstName']){
					

			 		$req = $pdo->prepare("UPDATE clients SET lastName= :lastName, firstName= :firstName, birthDate= :birthDate, card= :card, cardNumber= :cardNumber WHERE id=? ");
			 		$req->execute($donnee);
			 	}
			 }
			}
...
<body>
	...
	<form method="post" action="">
		<label>Quelle action souhaitez vous faire?</label>
		<select name="action">
